...
/* @breef discrete PID controller implementation with antiwindup
   ...
   for fixed point arithmetics call class PID<int> my_pid(kp, ki, kd, limit)
   for float arithmetics (default) call class PID<float> my_pid(kp, ki, kd, limit)
...
template <class int_type = float> class PID
{
  public:
    int_type e0, e1, e2;
    int_type k0, k1, k2;
  ...
  public:
    ...
    PID(int_type kp = 0, int_type ki = 0, int_type kd = 0, int_type output_range_ = 0)
    {
