...
uint16_t CRGB_I2C::init()
{
  ...
  */
  ...
  for (j = 0; j < RGB_SENSOR_COUNT; j++)
  {
    result_calibration[j].a = 0;
    result_calibration[j].r = 0;
    result_calibration[j].g = 0;
    result_calibration[j].b = 0;
  }

  for (j = 0; j < CALIBARATION_LOOPS; j++)
    read(true);

  for (j = 0; j < RGB_SENSOR_COUNT; j++)
  {
    result_calibration[j].a/= CALIBARATION_LOOPS;
    result_calibration[j].r/= CALIBARATION_LOOPS;
    result_calibration[j].g/= CALIBARATION_LOOPS;
    result_calibration[j].b/= CALIBARATION_LOOPS;
  }
...
void CRGB_I2C::read(bool calibration)
{
  unsigned char j;
  ...
  if (calibration)
  {
    for (j = 0; j < RGB_SENSOR_COUNT; j++)
    {
      result_calibration[j].a+= result[j].a;
      result_calibration[j].r+= result[j].r;
      result_calibration[j].g+= result[j].g;
      result_calibration[j].b+= result[j].b;
    }
  }
  else
  {
    for (j = 0; j < RGB_SENSOR_COUNT; j++)
    {
      result[j].a-= result_calibration[j].a;
      result[j].r-= result_calibration[j].r;
      result[j].g-= result_calibration[j].g;
      result[j].b-= result_calibration[j].b;
    }
  }
}
