...
template <unsigned int state_size, unsigned int actions_count, unsigned int hard_locations_count> class ReinforcementLearning
{
  protected:
    ...
    math_array<state_size> state, state_prev;
    associative_memory<state_size, actions_count, hard_locations_count> q;

    math_array<actions_count> q_values;
  ...
  public:
    ...
    unsigned int process(math_array<state_size> &state_)
    {
      state_prev = state;
      state = state_;
  ...
  private:
    ...
    void learn(math_array<state_size> &state_now_, unsigned int action_now_,
                math_array<state_size> &state_prev_, unsigned int action_prev_,
                float reward)
    {
      float gamma = 0.9;
